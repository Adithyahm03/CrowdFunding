...
export class EthcontractService {
  ...
  constructor() {
    try {
      if (typeof window.web3 !== 'undefined') {
        ...
        this.web3Provider = new Web3.providers.HttpProvider('http://127.0.0.1:8585');
        window.web3 = new Web3(this.web3Provider);
      }
      this.web3 = window.web3;

      const networkId = this.web3.currentProvider.networkVersion || '1772178886509'; // Fallback to current Ganache ID

      if (contract_SupplyChain.networks[networkId]) {
        this.contracts_SupplyChain = this.web3.eth.contract(
          contract_SupplyChain.abi
        ).at(contract_SupplyChain.networks[networkId].address);

        this.contractAddress_SupplyChain = contract_SupplyChain.networks[networkId].address;
        window.contracts = this.contracts_SupplyChain;
        this.getcoinbase();
        console.log('Contract loaded at:', this.contractAddress_SupplyChain);
      } else {
        console.warn('Contract not found on network:', networkId);
      }
    } catch (err) {
  ...
  getOwner = () => {
    ...
    return new Promise((resolve, reject) => {
      ...
      that.web3.eth.getAccounts(function (err, accounts) {
        if (err === null) {
          let account = accounts[0] || "0x0000000000000000000000000000000000000000";
          that.web3.eth.getBalance(account, function (err, balance) {
            const balanceVal = err === null ? that.web3.fromWei(balance, "ether") : "0";

            // If in demo mode as admin, return success regardless of contract owner
            if (sessionRole === 'admin') {
              console.log('EthcontractService: getOwner granted via sessionRole (Demo Mode)');
              return resolve({ Account: account, Balance: balanceVal, Role: 'Success', contractAddress: that.contractAddress_SupplyChain, UserCount: 10 });
            }
                ...
                if (!error) {
                  if (ownerAddress == account) {
                    that.contracts_SupplyChain.getUsersCount(function (error, userCount) {
                      if (!error) {
                        return resolve({ Account: account, Balance: balanceVal, Role: 'Success', contractAddress: that.contractAddress_SupplyChain, UserCount: JSON.parse(userCount) });
                      }
                      else
                        return resolve({ Account: account, Balance: balanceVal, Role: 'Success', contractAddress: that.contractAddress_SupplyChain, UserCount: "Error" });
                    })
                  }
                  else {
          ...
          return reject("No Coinbase/Account!");
        }
      });
    });
  }
            ...
            if (err === null) {
              ...
              that.contracts_SupplyChain.getUserInfo(
                account,
                {
                  ...
                  // Demo/Session override for non-admin roles
                  const sessionRole = localStorage.getItem('sessionRole');
                  ...
                  if (sessionRole && roleMapping[sessionRole]) {
                    console.log('EthcontractService: getRole granted via sessionRole (Demo Mode)');
                    return resolve({
                      Account: account,
                      Balance: that.web3.fromWei(balance, "ether"),
                      Role: {
                        "Name": sessionRole.toUpperCase(),
                        "Location": "DEMO_NODE",
                        "EthAddress": account,
                        "Role": roleMapping[sessionRole]
                      }
                    });
                  }
