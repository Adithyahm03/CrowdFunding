...
from .utils import create_order  # or just from .views if it's in same file
...
def your_view(request):
    try:
        # 👇 whichever code is running
        razorpay_order = razorpay_client.order.create({...})
        ...
    except Exception as e:
        return HttpResponse(f"<h1>View Error</h1><pre>{str(e)}</pre>")
...
def home(request):
    try:
        ...
        return HttpResponse(f"<h1>Error in View</h1><pre>{str(e)}</pre>")
